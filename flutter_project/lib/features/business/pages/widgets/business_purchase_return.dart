...
class _BusinessPurchaseReturnState extends State<BusinessPurchaseReturn> {
  ...
  Future<void> _loadData() async {
    try {
      final results = await Future.wait([
        ApiService.collection('business_suppliers').doc(widget.uid).collection('items').get(),
        ...
        ApiService.collection('business_invoices')
            ...
            .where('type', isEqualTo: 'purchase_return')
      ...
      if (mounted) {
        setState(() {
          _suppliers = (results[0] as QuerySnapshot).docs.map((d) {
  ...
  Future<void> _saveReturn() async {
    ...
    if (_selectedSupplierName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('حدد اسم المورد'), backgroundColor: Colors.orange)
      );
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تم حفظ مردود المشتريات بنجاح'), backgroundColor: Colors.green)
        );
        _resetForm();
        _loadData();
      }
    ...
    return showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(children: [
          Icon(Icons.redo, color: Color(0xFF0071E3), size: 22),
          SizedBox(width: 10),
          Text('طريقة استرجاع المبلغ', style: TextStyle(color: Color(0xFF0071E3), fontSize: 12)),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('كيف سيتم استرجاع المبلغ من المورد؟', style: TextStyle(color: Colors.white70, fontSize: 13)),
  ...
  Widget _buildToolbar() {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.redo, color: Colors.teal, size: 22),
          const SizedBox(width: 8),
          const Text('مردود مشتريات', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
          const Spacer(),
          Container(
            ...
            child: Text('الإجمالي: ${_totalAmount.toStringAsFixed(3)} د.ك', style: const TextStyle(color: Colors.teal, fontSize: 12, fontWeight: FontWeight.bold)),
          ),
        ],
      )
    );
        ...
        child: Column(
          ...
          children: [
            const Row(children: [
              ...
              SizedBox(width: 8),
              Text('مردود مشتريات جديد', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
            ]),
            ...
            TextField(
              ...
              style: TextStyle(
                color: _selectedSupplierId != null ? const Color(0xFF0071E3) : Colors.white,
                fontSize: 13,
              ),
              decoration: InputDecoration(
                labelText: 'اسم المورد',
                labelStyle: const TextStyle(color: Colors.white54, fontSize: 13),
                prefixIcon: const Icon(Icons.store, color: Colors.white38, size: 14),
                suffixIcon: _selectedSupplierId != null
                    ? IconButton(icon: const Icon(Icons.close, color: Colors.redAccent, size: 16), onPressed: () => setState(() { _selectedSupplierId = null; _selectedSupplierName = ''; _supplierNameController.clear(); }))
                    : null,
                filled: true,
              ...
              Container(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      dense: true,
                      leading: const Icon(Icons.store, color: Color(0xFF0071E3), size: 16),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('المنتجات المرتجعة', style: TextStyle(color: Colors.teal, fontSize: 12, fontWeight: FontWeight.bold)),
              ...
              child: ElevatedButton(
                ...
                child: _saving
                    ...
                    : const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        ...
                        SizedBox(width: 8),
                        Text('حفظ مردود المشتريات', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                      ]),
              ),
            ),
          ],
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            ...
            child: Autocomplete<Map<String, dynamic>>(
              ...
              optionsViewBuilder: (_, onSelected, options) => Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF1D1D1F), border: Border.all(color: Colors.teal.withOpacity(0.3)), borderRadius: BorderRadius.circular(6)),
          ...
          const SizedBox(width: 4),
          SizedBox(width: 80, child: Text('${item.total.toStringAsFixed(3)}', style: const TextStyle(color: Colors.teal, fontSize: 12, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
          if (_items.length > 1)
        ...
        children: [
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 6),
              Text('سجل مردودات المشتريات', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
            ]),
          ),
          Expanded(
  ...
  Widget _buildReturnCard(Map<String, dynamic> ret) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white.withOpacity(0.04),
        borderRadius: BorderRadius.circular(8),
        border: Border(right: const BorderSide(color: Colors.teal, width: 3)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            const Icon(Icons.redo, color: Colors.teal, size: 16),
            const SizedBox(width: 6),
            const Text('مردود مشتريات', style: TextStyle(color: Colors.teal, fontSize: 11, fontWeight: FontWeight.w600)),
            const Spacer(),
            Text('${amount.toStringAsFixed(3)} د.ك', style: const TextStyle(color: Colors.teal, fontSize: 14, fontWeight: FontWeight.bold)),
          ]),
          const SizedBox(height: 4),
          Row(children: [const Icon(Icons.store, color: Colors.white38, size: 12), const SizedBox(width: 4), Text(supplierName, style: const TextStyle(color: Colors.white70, fontSize: 12))]),
