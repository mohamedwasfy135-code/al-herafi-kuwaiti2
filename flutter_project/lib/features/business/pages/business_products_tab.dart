  ...
  Future<void> _showAddProductDialog() async {
    ...
    final result = await showDialog<Map<String, String>>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          const Icon(Icons.inventory_2, color: _C.accent, size: 22),
          const SizedBox(width: 10),
          const Text('إضافة مادة جديدة', style: TextStyle(color: _C.textP, fontSize: 20)),
        ]),
        content: SizedBox(
          width: 400,
          child: Column(
  ...
  Future<void> _editProduct(Map<String, dynamic> product) async {
    ...
    final result = await showDialog<Map<String, String>>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
        title: Row(children: [
          const Icon(Icons.edit, color: _C.accent, size: 22),
          const SizedBox(width: 10),
          const Text('تعديل المادة', style: TextStyle(color: _C.textP, fontSize: 20)),
        ]),
        content: SizedBox(
          width: 400,
          child: Column(
    ...
    if (result != null) {
      try {
        final double price = double.tryParse(result['price'] ?? '') ?? 0;
        final int stock = int.tryParse(result['stock'] ?? '') ?? 0;
        await ApiService.put('/api/products/${product['id']}', body: {
          'name': result['name'],
          'price': price,
          'category': result['category'],
          'description': result['description'],
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم تعديل المادة بنجاح'), backgroundColor: _C.success),
          );
        }
      } catch (e) {
        if (mounted) {
  ...
  Future<void> _deleteProduct(Map<String, dynamic> product) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
        title: Row(children: [
          const Icon(Icons.delete, color: _C.danger, size: 22),
          const SizedBox(width: 10),
          const Text('حذف المادة', style: TextStyle(color: _C.danger, fontSize: 20)),
        ]),
        content: Text('هل تريد حذف "${product['name']}" نهائياً؟', style: const TextStyle(color: _C.textP, fontSize: 18)),
        actions: [
    ...
    if (confirm == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم حذف المادة'), backgroundColor: _C.success),
          );
        }
      } catch (e) {
        if (mounted) {
    ...
    return Column(
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              const Icon(Icons.inventory_2, color: _C.accent, size: 20),
              const SizedBox(width: 8),
              const Text('المنتجات', style: TextStyle(color: _C.textP, fontSize: 14, fontWeight: FontWeight.bold)),
              const Spacer(),
              SizedBox(
                width: 220,
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: const TextStyle(color: _C.textM, fontSize: 13),
                    prefixIcon: const Icon(Icons.search, color: _C.textM, size: 14),
                    filled: true,
                    fillColor: _C.inputFill,
                    border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _C.border)),
                    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _C.border)),
        ...
        Expanded(
          child: FutureBuilder<List<Map<String, dynamic>>>(
            future: FirestoreService.getProducts(businessId: widget.uid),
            builder: (_, snap) {
              ...
              if (docs.isEmpty) {
                return Center(
                  child: Column(
                    ...
                    children: [
                      const Icon(Icons.inventory_2_outlined, size: 80, color: _C.textM),
                      ...
                      ElevatedButton.icon(
                        onPressed: _showAddProductDialog,
                        icon: const Icon(Icons.add_circle, size: 20),
                        label: const Text('أضف مادة جديدة', style: TextStyle(fontSize: 18)),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: _C.accent,
                          foregroundColor: Colors.white,
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
              ...
              int lowStockCount = 0;
              for (final data in docs) {
                final stock = (data['stockQuantity'] as num?)?.toInt() ?? -1;
                ...
                children: [
                  ...
                  if (lowStockCount > 0)
                    Container(
                      ...
                      child: Row(
                        children: [
                          const Icon(Icons.warning_amber, color: _C.warning, size: 14),
                          const SizedBox(width: 8),
                          Text(
                            '$lowStockCount مادة لديها مخزون منخفض أو نفد',
                            style: const TextStyle(color: _C.warning, fontSize: 13, fontWeight: FontWeight.w600),
                          ),
                        ],
                      ),
                    ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        Text('${docs.length} مادة', style: const TextStyle(color: _C.textM, fontSize: 12)),
                      ],
                    ),
                  ),
                  // شبكة البطاقات
  ...
  Widget _buildCategoryChip(String label, String? value) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected ? _C.accentLight : _C.surface,
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isOut
              ...
              : (isLow ? _C.warning.withOpacity(0.3) : (isActive ? _C.border : _C.danger.withOpacity(0.2))),
        ),
        boxShadow: [
          BoxShadow(
              ...
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: isOut
                            ...
                            : (isLow ? _C.warning.withOpacity(0.08) : _C.accentLight),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Icon(
                    ...
                    Expanded(
                      child: Text(
                        name,
                        style: TextStyle(
                          color: isActive ? _C.textP : _C.textM,
                          fontSize: 13,
                    ...
                    PopupMenuButton<String>(
                      ...
                      color: _C.bg,
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8), side: const BorderSide(color: _C.border)),
                      onSelected: (v) {
                      ...
                      itemBuilder: (_) => [
                        PopupMenuItem(value: 'edit', height: 36, child: Row(children: [const Icon(Icons.edit, color: _C.accent, size: 16), const SizedBox(width: 8), const Text('تعديل', style: TextStyle(color: _C.textS, fontSize: 13))])),
                        PopupMenuItem(value: 'toggle', height: 36, child: Row(children: [Icon(isActive ? Icons.visibility_off : Icons.visibility, color: _C.warning, size: 16), const SizedBox(width: 8), Text(isActive ? 'إلغاء التفعيل' : 'تفعيل', style: const TextStyle(color: _C.textS, fontSize: 13))])),
                        PopupMenuItem(value: 'delete', height: 36, child: Row(children: [const Icon(Icons.delete, color: _C.danger, size: 16), const SizedBox(width: 8), const Text('حذف', style: TextStyle(color: _C.danger, fontSize: 13))])),
                      ],
                      child: const Icon(Icons.more_vert, color: _C.textM, size: 14),
                    ),
                  ],
                ),
                ...
                if (category.isNotEmpty)
                  Container(
                    ...
                    child: Text(category, style: const TextStyle(color: _C.textM, fontSize: 14)),
                  ),
                const Spacer(),
                // المخزون والسعر
                Row(
                    ...
                    if (stock >= 0)
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isOut
                              ? _C.danger.withOpacity(0.08)
                              : (isLow ? _C.warning.withOpacity(0.08) : _C.success.withOpacity(0.08)),
                          borderRadius: BorderRadius.circular(4),
                          border: Border.all(
                            color: isOut
                                ? _C.danger.withOpacity(0.2)
                                : (isLow ? _C.warning.withOpacity(0.2) : _C.success.withOpacity(0.15)),
                          ),
                        ),
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(
                              ...
                              color: isOut ? _C.danger : (isLow ? _C.warning : _C.success),
                              size: 10,
                            ),
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          ...
                          style: const TextStyle(color: _C.textP, fontSize: 20, fontWeight: FontWeight.bold),
                        ),
                        const SizedBox(width: 4),
                        const Text('د.ك', style: TextStyle(color: _C.textS, fontSize: 12)),
                      ],
                    ),
                  ],
                ),
                ...
                if (description.isNotEmpty)
                  Padding(
                    ...
                    child: Text(description, style: const TextStyle(color: _C.textM, fontSize: 14), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ),
              ],
            ),
          ),
