#!/bin/bash
# ============================================================
# سكريبت إضافة مرحلة "بدء العمل"
# ============================================================

set -e

echo "🚀 بدء إضافة مرحلة 'بدء العمل'..."
echo "=========================================="

# ═══════════════════════════════════════════════════════════════
# 1️ API بدء العمل (للحرفي)
# ═══════════════════════════════════════════════════════════════
echo " إنشاء API بدء العمل..."
mkdir -p src/app/api/craftsman/start-work

cat << 'EOF' > src/app/api/craftsman/start-work/route.ts
...
echo "✅ تم إنشاء API بدء العمل"

# ══════════════════════════════════════════════════════════════
# 2️ تحديث واجهة الحرفي - إضافة زر "بدء العمل"
# ═══════════════════════════════════════════════════════════════
...
export default function CraftsmanDashboard() {
  ...
  // ✅ دالة جديدة: بدء العمل
  const handleStartWork = async (requestId: number) => {
    if (!confirm('هل أنت متأكد من بدء العمل على هذا الطلب؟\nسيتم إشعار العميل بأنك في الطريق.')) return
    setActionLoading(requestId)
    try {
                  ...
                  {myRequests.map((req: any) => (
                    <div key={req.id} className="bg-white border rounded-lg p-4 hover:shadow-md transition">
                      ...
                      {req.status === 'accepted' && (
                        ...
                        >
                          {actionLoading === req.id ? 'جاري...' : '🔨 بدء العمل'}
                        </button>
                      )}
...
# ═══════════════════════════════════════════════════════════════
# 3️⃣ تحديث واجهة العميل - إضافة حالة "قيد التنفيذ"
# ═══════════════════════════════════════════════════════════════
                  ...
                  <div key={req.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                    ...
                    {req.status === 'in_progress' && (
                      <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                        <p className="text-sm text-blue-800 font-bold">🚗 الحرفي في الطريق إليك! سيتواصل معك قريباً.</p>
                      </div>
                    )}

                    {req.status === 'accepted' && (
                      <div className="mt-3 p-3 bg-indigo-50 rounded-lg border border-indigo-200">
                        <p className="text-sm text-indigo-800 font-bold">✅ تم قبول طلبك. الحرفي سيبدأ العمل قريباً.</p>
                      </div>
                    )}
...
# ═══════════════════════════════════════════════════════════════
echo "📦 حفظ التغييرات..."
git add src/app/api/craftsman/start-work/route.ts \
        src/app/craftsman/dashboard/page.tsx \
        src/app/dashboard/client/page.tsx

git commit -m "feat: إضافة مرحلة 'بدء العمل' في دورة حياة الطلب

- API جديد: /api/craftsman/start-work
- زر 'بدء العمل' للحرفي في تبويب طلباتي (يظهر فقط للطلبات المقبولة)
- إشعار تلقائي للعميل: 'الحرفي في الطريق'
- تحديث واجهة العميل لعرض حالة 'in_progress'
- رسائل توضيحية للعميل حسب حالة الطلب
- تحسين عرض حالات الطلب بأيقونات ملونة"
...
echo "=========================================="
echo ""
echo "📋 التدفق الجديد للطلب:"
echo "1. العميل ينشئ طلب → pending"
echo "2. الحرفي يقبل → accepted"
echo "3. الحرفي يضغط 'بدء العمل' → in_progress (جديد!)"
echo "4. الحرفي يضغط 'إتمام العمل' → completed"
echo "5. العميل يدفع → paid"
echo "6. العميل يقيّم"
echo ""
echo "🧪 جرب الآن:"
echo "- سجّل دخول كحرفي → اذهب لطلباتي → اضغط 'بدء العمل'"
echo "- سجّل دخول كعميل → اذهب لطلباتي → سترى 'الحرفي في الطريق'"
echo ""
echo "🚀 جاهز للاختبار!"
