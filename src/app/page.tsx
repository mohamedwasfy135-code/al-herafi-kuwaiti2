...
// دالة لعرض الأيقونة بنفس أسلوب صفحة العميل
const ServiceIcon = ({ name }: { name?: string }) => {
  if (!name) return <span className="text-4xl">🛠️</span>
  
  // إذا كان إيموجي
  if (/[^\x00-\x7F]/.test(name)) {
    return <span className="text-4xl">{name}</span>
  }

  // تحويل الاسم إلى مكون Lucide
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12 py-8">
          ...
          <p className="text-lg text-gray-600">
            اطلب خدمات الحرفيين أو تسوق من المحلات بسهولة وأمان
          </p>
        </div>

        {/* Services Section - نفس تصميم صفحة العميل */}
